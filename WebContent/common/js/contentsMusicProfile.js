...
//상세페이지 이동 함수
function go_detailPage_Music(target_CSEQ){
	location.href = get_domain() + '/music/contentMusic.html?cseq=' + target_CSEQ;
}

//리뷰 리스트를 ajax요청하는 함수
function getReviewList(currentPage){
	var mseq = location.search.split('?mseq=')[1];
	var URL = '/contents/contentProfileReviewList.html';
	$.ajax({
		type: 'GET',
		url: URL,
		dataType: 'html',
